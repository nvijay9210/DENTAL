...
exports.getAllToothDetailssByTenantId = async (req, res, next) => {
  ...
  try {
    ...
    const toothdetailss = await toothdetailsService.getAllToothDetailssByTenantId(tenant_id, page, limit);
...
exports.getToothDetailsByTenantIdAndToothDetailsId = async (req, res, next) => {
  ...
  try {
    const toothdetails1=await checkIfExists('toothdetails','toothdetails_id',toothdetails_id,tenant_id);
    if(!toothdetails1) throw new CustomError('ToothDetails not found',404)

    // Fetch toothdetails details
    const toothdetails = await toothdetailsService.getToothDetailsByTenantIdAndToothDetailsId(
      tenant_id,
      toothdetails_id
    );
...
exports.updateToothDetails = async (req, res, next) => {
  ...
  try {
    ...
    await toothdetailsValidation.updateToothDetailsValidation(toothdetails_id, details);

    // Update the toothdetails
    await toothdetailsService.updateToothDetails(toothdetails_id, details, tenant_id);
 ...
 */
exports.deleteToothDetailsByTenantIdAndToothDetailsId = async (req, res, next) => {
  const { toothdetails_id, tenant_id } = req.params;
  ...
  try {
    ...
    await toothdetailsService.deleteToothDetailsByTenantIdAndToothDetailsId(tenant_id, toothdetails_id);
