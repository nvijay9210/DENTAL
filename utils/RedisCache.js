...
function buildCacheKey(type, scope, options = {}) {
  ...
  if (supplier_product_id) parts.push(`supplier_product_id:${supplier_product_id}`);
  if (supplier_payment_id) parts.push(`supplier_payment_id:${supplier_payment_id}`);
  if (supplier_review_id) parts.push(`supplier_review_id:${supplier_review_id}`);
